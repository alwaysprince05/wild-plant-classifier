...
def load_model():
    ...
    try:
        ...
        print("⚠️ Warning: Model weights 'saved_models/best_resnet34.pt' not found.")
        print("   Running in UI-only mode with mock predictions.")
        model = None
        return {'accuracy': 0.8067}
...
def predict_image(image_bytes):
    ...
    if model is None:
        # Return mock predictions if model didn't load
        import random
        selected = random.sample(PLANT_CLASSES, 5)
        predictions = []
        conf = 95.5
        for cls in selected:
            predictions.append({
