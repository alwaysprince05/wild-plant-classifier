...
![Plant Classes](imgs/plant-classes.png "Wild Edible Plant Classes")

_Figure 1. The 35 Wild Edible Plant classes used in the project._

The classes of wild edible plants used are listed in table 1, accompanied by the number of images (per class) within the dataset. The dataset comprised of Flickr images, is obtained through their API using the rudimentary scripts within the `\data_gathering` folder. The dataset contains 16,535 images, where the quantity of images per class varies from 400 to 500.
...
![ML Pipeline](imgs/ml-pipeline.png "Machine Learning Pipeline")
...
2. Clone the repository, navigate to the `wep-classifier/` folder and install the required dependencies.
    ...
    ```bash
    git clone https://github.com/alwaysprince05/wild-plant-classifier.git
    cd wild-plant-classifier
